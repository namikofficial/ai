...
import type { CompiledPromptRecord, ConfigSnapshot, ProjectSummary } from "../../../../packages/shared/src/index.ts";
...
export function renderProjectsPage(store: Store): string {
  ...
  return pageShell("Projects", "/projects", {
    ...
    rightPanelHtml: renderCard(
      "Quick Tips",
      `<div class="tiny">Project indexing scans text files, stores chunks, and immediately emits a session trace.</div>`
    ),
    projects,
...
export function renderTaskDetailPage(store: Store, taskId: string): string {
  ...
  const events = store
    .listEvents(task.sessionId, 100)
    .filter((event) => event.taskId === task.id || event.taskId == null);
  ...
  return pageShell(task.title, `/tasks/${task.id}`, {
    contentHtml: [
      ...
      renderCard(
        "Actions",
        `<div class="stack"><form method="post" action="/tasks/${encodeURIComponent(task.id)}/start"><button type="submit">Start task</button></form><form method="post" action="/tasks/${encodeURIComponent(task.id)}/complete" class="stack"><textarea name="result" placeholder="completion notes"></textarea><button type="submit">Complete task</button></form><form method="post" action="/tasks/${encodeURIComponent(task.id)}/fail" class="stack"><textarea name="error" placeholder="failure notes"></textarea><button type="submit">Fail task</button></form></div>`,
        6
      ),
      renderCard(
        "Expected Files",
        expectedFiles.length > 0
          ? `<div class="list">${expectedFiles.map((file) => `<div class="list-item">${escapeHtml(file)}</div>`).join("")}</div>`
          : renderEmptyState("No expected files", "Planner-created tasks will list files here."),
        4
      ),
      renderCard(
        "Actual Files",
        actualFiles.length > 0
          ? `<div class="list">${actualFiles.map((file) => `<div class="list-item">${escapeHtml(file)}</div>`).join("")}</div>`
          : renderEmptyState("No actual files", "Update the task after edits land."),
        4
      ),
      renderCard(
        "Checks",
        checks.length > 0
          ? `<div class="list">${checks.map((check) => `<div class="list-item">${escapeHtml(check)}</div>`).join("")}</div>`
          : renderEmptyState("No checks", "Checks will be attached to the task graph."),
        4
      ),
...
export function renderAskPage(
  store: Store,
  options: { result?: Awaited<ReturnType<Store["ask"]>>; error?: string; question?: string } = {}
): string {
  const projects = store.listProjects();
  const selectOptions = projects
    .map(
      (project) =>
        `<option value="${escapeHtml(project.id)}"${options.result?.projectId === project.id ? " selected" : ""}>${escapeHtml(project.name)}</option>`
    )
    .join("");
    ...
    contentHtml: [
      ...
      renderCard(
        ...
        options.result
          ...
          : renderEmptyState(
              "No answer yet",
              options.error ?? "Submit a question to see retrieved context and citations."
            ),
        6
      ),
      renderCard(
        ...
        options.result && options.result.citations.length > 0
          ? `<div class="list">${options.result.citations.map((citation) => `<div class="list-item"><strong>${escapeHtml(citation.path)}</strong><div class="tiny">Lines ${citation.startLine}-${citation.endLine} · score ${citation.score.toFixed(1)}</div><pre>${escapeHtml(citation.excerpt)}</pre></div>`).join("")}</div>`
...
export function renderResearchPage(
  ...
  const projectOptions = projects
    .map(
      (project) =>
        `<option value="${escapeHtml(project.id)}"${options.projectId === project.id ? " selected" : ""}>${escapeHtml(project.name)}</option>`
    )
    .join("");
  return pageShell("Research", "/research", {
    contentHtml: [
      ...
      renderCard(
        "Summary",
        options.result
          ? `<pre>${escapeHtml(options.result.summary)}</pre>`
          : renderEmptyState("No research yet", "Run a topic search to gather a brief."),
        6
      ),
      renderCard(
        "Sources",
      ...
      renderCard(
        "Brief",
        options.result
          ? `<pre>${escapeHtml(options.result.brief)}</pre>`
          : renderEmptyState("No brief yet", "The final brief will be suitable for handoff."),
        6
      ),
    ].join(""),
    rightPanelHtml: renderCard("Recent Trace", renderEventFeed(store.listEvents(undefined, 40))),
...
export function renderPlannerPage(
  store: Store,
  options: { result?: Awaited<ReturnType<Store["createPlan"]>>["response"]; error?: string } = {}
): string {
  const projects = store.listProjects();
  const projectOptions = projects
  ...
  return pageShell("Planner", "/planner", {
    contentHtml: [
      ...
      renderCard(
        "Task Graph",
        options.result
          ? `<div class="list">${options.result.taskGraph.map((task) => `<div class="list-item"><div class="row"><strong>${escapeHtml(task.title)}</strong><span class="badge">${escapeHtml(task.status)}</span></div><div class="tiny">${escapeHtml(task.description)}</div><div class="tiny">Checks: ${escapeHtml(task.checks.join(", "))}</div><div class="tiny">Files: ${escapeHtml(task.expectedFiles.join(", ") || "none")}</div></div>`).join("")}</div>`
...
export function renderHandoffPage(
  store: Store,
  options: { result?: Awaited<ReturnType<Store["createHandoff"]>>; error?: string } = {}
): string {
  const projects = store.listProjects();
...
export function renderChecksPage(store: Store): string {
  ...
  return pageShell("Checks", "/checks", {
    contentHtml: [
      renderCard(
        "Allowed Checks",
        `<div class="list">${["typecheck", "tests", "build", "lint"].map((name) => `<div class="list-item"><strong>${name}</strong><div class="tiny">Allowlisted validation check</div></div>`).join("")}</div>`,
        4
      ),
      renderCard(
        "Recent Runs",
      ...
      renderCard(
        "Run Check",
        `<form method="post" action="/checks/run" class="stack"><input name="name" placeholder="typecheck" /><input name="projectId" placeholder="optional project id" /><button type="submit">Record check run</button></form>`,
        4
      ),
    ].join(""),
    rightPanelHtml: renderCard("Recent Events", renderEventFeed(store.listEvents(undefined, 40))),
...
export function renderReviewsPage(
  store: Store,
  options: { result?: ReturnType<Store["createReview"]>; error?: string } = {}
): string {
  const projects = store.listProjects();
  const projectOptions = projects
    .map((project) => `<option value="${escapeHtml(project.id)}">${escapeHtml(project.name)}</option>`)
    ...
    contentHtml: [
      ...
      renderCard(
        ...
        options.result
          ...
          : renderEmptyState(
              "No review yet",
              options.error ?? "Create a review to capture scope creep, missing tests, and risks."
            ),
        6
      ),
      renderCard(
    ...
    contentHtml: [
      ...
      renderCard(
        ...
        plannedFiles.length > 0
          ? `<div class="list">${plannedFiles.map((file) => `<div class="list-item">${escapeHtml(file)}</div>`).join("")}</div>`
          : renderEmptyState("No planned files", "The review did not capture planned files."),
        4
      ),
      renderCard(
        "Edited Files",
        editedFiles.length > 0
          ? `<div class="list">${editedFiles.map((file) => `<div class="list-item">${escapeHtml(file)}</div>`).join("")}</div>`
          : renderEmptyState("No edited files", "The review did not capture edited files."),
        4
      ),
      renderCard(
        "Checks",
        checks.length > 0
          ? `<div class="list">${checks.map((check) => `<div class="list-item">${escapeHtml(check)}</div>`).join("")}</div>`
          : renderEmptyState("No checks", "The review did not capture validation checks."),
        4
      ),
      renderCard(
        "Scope Creep",
        scopeCreep.length > 0
          ? `<div class="list">${scopeCreep.map((file) => `<div class="list-item">${escapeHtml(file)}</div>`).join("")}</div>`
          : renderEmptyState("No scope creep", "Nothing extra slipped into the change set."),
        4
      ),
      renderCard(
        "Missing Tests",
        missingTests.length > 0
          ? `<div class="list">${missingTests.map((file) => `<div class="list-item">${escapeHtml(file)}</div>`).join("")}</div>`
          : renderEmptyState("No missing tests", "The review did not flag missing tests."),
        4
      ),
      renderCard(
        "Risky Changes",
        riskyChanges.length > 0
          ? `<div class="list">${riskyChanges.map((file) => `<div class="list-item">${escapeHtml(file)}</div>`).join("")}</div>`
          : renderEmptyState("No risky changes", "Nothing high-risk was detected."),
        4
      ),
    ].join(""),
...
export function renderRetrievalPage(
  store: Store,
  options: { projectId?: string; query?: string; chunks?: ReturnType<Store["searchChunks"]> } = {}
): string {
  const projects = store.listProjects();
  const projectOptions = projects
    .map(
  ...
  return pageShell("Retrieval", "/retrieval", {
    contentHtml: [
      renderCard(
        "Search Retrieval",
        `<form method="post" action="/retrieval/search" class="stack"><select name="project">${projectOptions || `<option value="">Add a project first</option>`}</select><textarea name="query" placeholder="where is auth handled?">${escapeHtml(options.query ?? "")}</textarea><button type="submit">Search</button></form>`,
        6
      ),
      renderCard(
        "Results",
        chunks.length > 0
          ? `<div class="list">${chunks.map((chunk) => `<div class="list-item"><div class="row"><strong>${escapeHtml(chunk.path)}</strong><span class="badge">score ${chunk.score.toFixed(1)}</span></div><div class="tiny">Lines ${chunk.startLine}-${chunk.endLine}</div><pre>${escapeHtml(chunk.content.slice(0, 260))}</pre></div>`).join("")}</div>`
...
export function renderModelsPage(store: Store, config: ConfigSnapshot): string {
  ...
  return pageShell("Models", "/models", {
    ...
    rightPanelHtml: renderCard(
      "Config",
      `<pre>${escapeHtml(JSON.stringify(store.getSettings(config), null, 2))}</pre>`
    ),
    projects: store.listProjects(),
    projectCount: store.listProjects().length,
    sessionCount: store.listSessions(1000).length,
    ...
    contentHtml: [
      renderCard(
        ...
        renderKeyValueList([
          [
            "Allowed tools",
            "ai_search_project, ai_ask_rag, ai_create_session, ai_create_plan, ai_get_current_task, ai_get_next_subtask, ai_create_handoff, ai_run_check",
          ],
  ...
  return pageShell("Settings", "/settings", {
    contentHtml: [
      ...
      renderCard(
        ...
        renderEmptyState(
          "Current defaults",
          "The bootstrap uses local SQLite, local runtime directories, and no cloud routing by default."
        ),
        12
      ),
    ].join(""),
