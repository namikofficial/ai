...
import { renderMemoryPage } from "./ssr.ts";
...
export function registerMemoryRoutes(router: Router, deps: { store: Store }) {
  ...
  router.post("/memory/lesson", asyncRoute(async (req, res) => {
    const body = (
  ...
  router.post("/memory/reflect", asyncRoute(async (req, res) => {
    const body = (
      req.headers["content-type"]?.includes("application/json")
        ? await readJsonBody(req)
        : Object.fromEntries(new URLSearchParams(await readTextBody(req)))
    ...
    deps.store.appendEvent(createEvent("lesson.created", { title: lesson.title, body: lesson.body, tags: ["reflection"], importance: 3 }, { sessionId, projectId: session.projectId, agent: "learning" }));
    if (isHtmlRequest(req)) {
      sendHtml(res, renderMemoryPage(deps.store));
      return;
    }
  ...
  router.get("/memory/candidates", (req, res) => {
    const status = typeof req.query.status === "string" ? req.query.status as "pending" | "accepted" | "rejected" : null;
  ...
  router.get("/memory/entries", (req, res) => {
    const projectId = typeof req.query.projectId === "string" ? req.query.projectId : null;
    const scope = typeof req.query.scope === "string" ? req.query.scope as "global" | "project" | "repo" | "path" : null;
