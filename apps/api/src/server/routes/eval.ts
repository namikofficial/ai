...
export function registerEvalRoutes(router: Router, deps: { store: Store }) {
  router.get("/eval/cases", (req, res) => {
    ...
    sendJson(res, json("ok", projectId ? deps.store.evals.listCases().filter((item) => item.projectId === projectId) : deps.store.evals.listCases()));
  });

  router.post("/eval/cases", asyncRoute(async (req, res) => {
    const body = (await readJsonBody(req)) as { projectId?: string; question: string; expectedAnswerContains?: string; expectedFiles?: string[]; tags?: string[] };
    if (!body.question) {
      sendJson(res, json("error", undefined, { message: "question is required" }), 400);
      return;
    }
    sendJson(res, json("ok", deps.store.evals.addCase({
  ...
  router.get("/eval/outcomes", (req, res) => {
    ...
    sendJson(res, json("ok", sessionId ? deps.store.evals.listOutcomes(sessionId) : deps.store.evals.listAllOutcomes(100)));
  });
}
