...
export function registerRetrievalRoutes(
  ...
  router.get("/retrieval", (req, res) => {
    if (!isHtmlRequest(req)) {
      sendJson(
        res,
        json("ok", { projects: deps.store.listProjects(), recentLessons: deps.store.listRecentLessons(20) })
      );
      return;
    }
  ...
  router.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      const explanation = deps.runExplainWithStore({ projectId, query, mode, depth, limit }) as {
        selected: Array<{ path: string; finalScore: number }>;
        ranked: Array<{ path: string; rerankReason?: string }>;
      };
      sendJson(
        res,
        json("ok", {
