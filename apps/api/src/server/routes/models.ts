...
export function registerModelRoutes(
  ...
  router.get("/models", (req, res) => {
    if (!isHtmlRequest(req)) {
      sendJson(
        res,
        json("ok", { usage: deps.store.listModelUsage(50), settings: deps.store.getSettings(deps.config) })
      );
      return;
    }
  ...
  router.get("/models/providers", (_req, res) => {
    sendJson(
      res,
      json("ok", { providers: deps.store.models.listProviders(), profiles: deps.store.models.listProfiles() })
    );
  });
  ...
  router.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      const removed = cache.purge({
        olderThanDays: body.olderThanDays == null ? undefined : Number(body.olderThanDays),
