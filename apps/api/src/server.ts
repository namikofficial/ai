      ...
      `<div class="list">${
        dashboard.recentLessons.length > 0
          ? dashboard.recentLessons
              .map(
                (lesson) =>
                  `<div class="list-item"><strong>${escapeHtml(lesson.title)}</strong><div class="tiny">${escapeHtml(lesson.body)}</div></div>`
              )
              .join("")
          : renderEmptyState("No lessons yet", "Answers and indexing runs will populate memory.")
      }</div>`,
      6
    ),
    renderCard(
      ...
      `<div class="list">${
        dashboard.recentChecks.length > 0
          ? dashboard.recentChecks
              .map(
                (check) =>
                  `<div class="list-item"><strong>${escapeHtml(check.name)}</strong><div class="tiny">${escapeHtml(check.status)}</div></div>`
              )
              .join("")
          : renderEmptyState("No checks yet", "Allowlisted checks will show up here.")
      }</div>`,
      6
    ),
  ].join("");
...
function renderAskPage(
  store: ReturnType<typeof createStore>,
  options: {
    result?: Awaited<ReturnType<ReturnType<typeof createStore>["ask"]>>;
    error?: string;
    question?: string;
  } = {}
): string {
...
function renderPlannerPage(
  store: ReturnType<typeof createStore>,
  options: {
    result?: Awaited<ReturnType<ReturnType<typeof createStore>["createPlan"]>>["response"];
    error?: string;
  } = {}
): string {
  const projects = store.listProjects();
    ...
    renderCard(
      ...
      options.result
        ? `<div class="list">${options.result.taskGraph
            .map(
              (task) =>
                `<div class="list-item"><div class="row"><strong>${escapeHtml(task.title)}</strong><span class="badge">${escapeHtml(task.status)}</span></div><div class="tiny">${escapeHtml(task.description)}</div><div class="tiny">Checks: ${escapeHtml(task.checks.join(", "))}</div><div class="tiny">Files: ${escapeHtml(task.expectedFiles.join(", ") || "none")}</div></div>`
            )
            .join("")}</div>`
...
function renderHandoffPage(
  store: ReturnType<typeof createStore>,
  options: {
    result?: Awaited<ReturnType<ReturnType<typeof createStore>["createHandoff"]>>;
    error?: string;
  } = {}
): string {
  const projects = store.listProjects();
  ...
  const contentHtml = [
    renderCard(
      ...
      `<div class="list">${["typecheck", "tests", "build", "lint"]
        .map(
          (name) =>
            `<div class="list-item"><strong>${name}</strong><div class="tiny">Allowlisted validation check</div></div>`
        )
        .join("")}</div>`,
      4
    ),
...
function renderReviewsPage(
  store: ReturnType<typeof createStore>,
  options: {
    result?: ReturnType<ReturnType<typeof createStore>["createReview"]>;
    error?: string;
  } = {}
): string {
  const projects = store.listProjects();
...
function renderRetrievalPage(
  ...
  options: {
    projectId?: string;
    query?: string;
    chunks?: ReturnType<ReturnType<typeof createStore>["searchChunks"]>;
  } = {}
): string {
  const projects = store.listProjects();
  const projectOptions = projects
    ...
    renderCard(
      ...
      chunks.length > 0
        ? `<div class="list">${chunks
            .map(
              (chunk) =>
                `<div class="list-item"><div class="row"><strong>${escapeHtml(chunk.path)}</strong><span class="badge">score ${chunk.score.toFixed(1)}</span></div><div class="tiny">Lines ${chunk.startLine}-${chunk.endLine}</div><pre>${escapeHtml(chunk.content.slice(0, 260))}</pre></div>`
            )
            .join("")}</div>`
    ...
    void (async () => {
      ...
      try {
        ...
        if (path.startsWith("/dev/")) {
          ...
          if (method === "POST" && head === "runs" && trimmedRest.endsWith("/cancel")) {
            ...
            sendJson(
              ...
              outcome.ok
                ? json("ok", outcome.run)
                : json("error", undefined, { message: outcome.error ?? "cancel failed" }),
              outcome.ok ? 200 : 400
            );
            return;
          }
        ...
        if (method === "POST" && path === "/checks/run") {
          ...
          if (!allowed.has(name)) {
            store.appendEvent(
              createEvent(
                "tool.blocked",
                { tool: name, reason: "check not allowlisted" },
            ...
            const engineResult = await runPromptLab(
              {
                ...
                getCompiledPrompt(id: string) {
                  return store.getCompiledPrompt(id);
                },
                createRun(input) {
                  return store.promptLab.createRun(input);
                },
                createResult(input) {
                  return store.promptLab.createResult(input);
                },
                getProfile(id: string) {
                  return store.models.getProfile(id);
                },
                listProfiles() {
                  return store.models.listProfiles();
                },
                listProviders() {
                  return store.models.listProviders();
                },
              },
              { projectId, promptId, selectedProfiles, notes, dryRun },
              {
                ...
                recordModelCall(input: Parameters<ModelCallRecordedHook>[0]) {
                  return store.models.recordCall(input);
                },
              }
            );
        ...
        if (method === "POST" && path === "/retrieval/search") {
          ...
          const chunks = store.searchChunks(projectId, query, {
          ...
          if (method === "GET") {
            ...
            sendHtml(
              ...
              pageShell(`MCP ${call.toolName}`, `/mcp/calls/${call.id}`, {
                ...
                rightPanelHtml: renderCard(
                  "Recent Trace",
                  renderEventFeed(store.listEvents(call.sessionId ?? undefined, 40))
                ),
                activeProjectId: project?.id ?? null,
                projects: store.listProjects(),
                projectCount: store.listProjects().length,
        ...
        if (method === "GET" && path === "/models/health") {
          ...
          sendJson(res, json("ok", { providers: providersWithHealth, recentCalls }));
          return;
        }
        ...
        if (path.startsWith("/agents/runs/") && method === "GET") {
          ...
          sendJson(res, json("ok", { run, messages: store.agents.listMessages(id) }));
          return;
        }
