...
| Execution-mode downgrade or wrong checkout | Approved commands own their execution mode. Terminal/tmux presentation may be selected only for an approved desktop-launch command; isolated/background commands cannot be caller-downgraded. Isolated commands remap a canonical relative working directory into a retained Git worktree or safe copy, and never execute in the original checkout. | Legacy mode normalization, invalid mode, downgrade rejection, and isolated original-tree regression tests. | Add namespace/container isolation for hostile build tools that can intentionally access paths outside their working directory. |
