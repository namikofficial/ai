...
test("worker processes queued plan jobs", async () => {
  ...
  const planSession = store
    .listProjectSessions(project.id, 50)
    .find((s) => s.userGoal.includes("reduce auth complexity"));
  ...
  assert.equal(
    store.listJobs(10).some((job) => job.type === "review.reflect"),
    true
  );
  assert.equal(await processNextJob(store), true);
  assert.equal(
    store.listJobs(10).some((job) => job.status === "queued" && job.type === "review.reflect"),
    false
  );
  assert.ok(
    store
  ...
  const handoffCandidates = store.memory
    .listCandidates("pending", project.id, 100)
    .filter((c) => c.sessionId === handoffSession.id);
  const handoffSkills = store.skills
    .listCandidates("pending", 100)
    .filter((c) => c.exampleSessionId === handoffSession.id);
  assert.ok(
