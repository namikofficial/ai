...
import {
  normalizeProfileIds,
  type PromptLabEngineStore,
  runPromptLab,
} from "../packages/prompt-lab-engine/src/index.ts";
import type {
...
test("runPromptLab normalizes profile IDs and validates messages before creating a run", async () => {
  const store = makeStore({
    compiledPrompt: makeCompiledPrompt('{"role":"user","content":"hello"}'),
  });
  try {
    await runPromptLab(
      store,
...
test("runPromptLab preserves provider baseUrl and apiKeyEnv and records successful model calls", async () => {
  ...
  try {
    ...
    assert.deepEqual(recordCalls[0]?.request.metadata, {
      source: "prompt-lab",
      promptId: "prompt-1",
      runId: result.run.id,
    });
