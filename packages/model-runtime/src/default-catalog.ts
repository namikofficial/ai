...
function readEnvOverrides(env: NodeJS.ProcessEnv = process.env): DefaultEnvOverrides {
  return {
    ...
    embeddingModel: env.AI_LOCAL_EMBEDDING_MODEL && env.AI_LOCAL_EMBEDDING_MODEL.length > 0 ? env.AI_LOCAL_EMBEDDING_MODEL : undefined,
  };
}
...
export function buildDefaultProfileRows(env: NodeJS.ProcessEnv = process.env): DefaultProfileInput[] {
  ...
  return [
    { id: "intent-local", providerId: local, role: "intent", modelName: fastModel, displayName: "Intent classifier (local)" },
    { id: "query-rewrite-local", providerId: local, role: "query_rewrite", modelName: fastModel, displayName: "Query rewriter (local)" },
    { id: "retrieval-judge-local", providerId: local, role: "retrieval_judge", modelName: fastModel, displayName: "Retrieval judge (local)" },
    { id: "reranker-local", providerId: local, role: "reranker", modelName: fastModel, displayName: "Reranker (local)" },
    { id: "embedding-local", providerId: local, role: "embedding", modelName: embeddingModel, displayName: "Embedding (local)" },
    { id: "summarizer-local", providerId: local, role: "summarizer", modelName: fastModel, displayName: "Summarizer (local)" },
    { id: "reviewer-local", providerId: local, role: "reviewer", modelName: fastModel, displayName: "Reviewer (local)" },
    { id: "reflection-local", providerId: local, role: "reflection", modelName: deepModel, displayName: "Reflection (local)" },
    { id: "indexer-local", providerId: local, role: "embedding", modelName: embeddingModel, displayName: "Indexer (local)" },
    { id: "ask-fast-local", providerId: local, role: "answer", modelName: fastModel, displayName: "Fast answer (local)" },
    { id: "ask-extended-local", providerId: local, role: "answer", modelName: deepModel, displayName: "Extended answer (local)", contextWindow: 16_384, maxOutputTokens: 4_096 },
    { id: "ask-deep-local", providerId: local, role: "answer", modelName: deepModel, displayName: "Deep answer (local)", contextWindow: 32_768, maxOutputTokens: 4_096 },
    { id: "ask-hybrid-router", providerId: local, role: "answer", modelName: fastModel, displayName: "Hybrid answer router (local)" },
    {
      id: "ask-cloud-router",
    ...
    { id: "planner-fast-local", providerId: local, role: "planner", modelName: fastModel, displayName: "Fast planner (local)" },
    { id: "planner-balanced-local", providerId: local, role: "planner", modelName: deepModel, displayName: "Balanced planner (local)" },
    { id: "planner-deep-local", providerId: local, role: "planner", modelName: deepModel, displayName: "Deep planner (local)", contextWindow: 32_768, maxOutputTokens: 4_096 },
    { id: "handoff-local", providerId: local, role: "coder_handoff", modelName: coderModel, displayName: "Handoff compiler (local)" },
    { id: "checker-local", providerId: local, role: "reviewer", modelName: fastModel, displayName: "Check summarizer (local)" },
    { id: "dev-editor-local", providerId: local, role: "coder_handoff", modelName: coderModel, displayName: "Dev editor (local)", contextWindow: 32_768, maxOutputTokens: 4_096 },
    { id: "dev-repair-local", providerId: local, role: "coder_handoff", modelName: coderModel, displayName: "Dev repair (local)", contextWindow: 32_768, maxOutputTokens: 4_096 },
...
export function buildDefaultProfile(profile: DefaultProfileInput): ModelProfileRecord {
  ...
  return {
    ...
    qualityScore: profile.qualityScore ?? (profile.role === "planner" ? 0.7 : profile.role === "answer" ? 0.65 : profile.role === "coder_handoff" ? 0.7 : 0.6),
