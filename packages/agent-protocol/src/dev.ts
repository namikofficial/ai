...
export function parseDevRequest(value: unknown): DevRequest {
  ...
  return {
    ...
    approvalPolicy:
      value.approvalPolicy == null ? "manual" : oneOf(value.approvalPolicy, APPROVAL_POLICIES, "approvalPolicy"),
...
export function parseDevCheckResult(value: unknown): DevCheckResult {
  ...
  const status =
    value.status == null
      ? "completed"
      : oneOf(value.status, ["queued", "running", "completed", "failed", "blocked"] as const, "status");
