  ...
  return {
    get(key) {
      const row = db
        ...
        .get(key.providerId, key.modelName, key.dimension, key.contentHash) as
        | CachedRow
        | undefined;
      if (!row) return null;
    ...
    touch(id) {
      db.prepare(
        "UPDATE embedding_cache SET last_used_at = ? WHERE id = ?",
      ).run(now(), id);
    },
    recordHit(id) {
      db.prepare(
        "UPDATE embedding_cache SET hit_count = hit_count + 1, last_used_at = ? WHERE id = ?",
      ).run(now(), id);
      const row = db
        .prepare(
          "SELECT provider_id, model_name, dimension FROM embedding_cache WHERE id = ?",
        )
        .get(id) as
    ...
    purge(opts) {
      if (opts.olderThanDays && opts.olderThanDays > 0) {
        ...
        const result = db
          .prepare("DELETE FROM embedding_cache WHERE last_used_at < ?")
          .run(cutoff);
      ...
      const result = db
        .prepare(
          `DELETE FROM embedding_cache WHERE ${conditions.join(" AND ")}`,
        )
        .run(...args);
      return Number(result.changes);
    },
